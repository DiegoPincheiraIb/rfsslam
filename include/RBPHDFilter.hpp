...
public:
   ...
   * \todo predict the map as well with its static motion model
   */
...
void RBPHDFilter< RobotProcessModel, LmkProcessModel, MeasurementModel, KalmanFilter >::updateMap(){
  ...
  for(int i = 0; i < this->nParticles_; i++){
	...
	if ( innovationLikelihood < config.newGaussianCreateLikelihoodThreshold_ ){
...
void RBPHDFilter< RobotProcessModel, LmkProcessModel, MeasurementModel, KalmanFilter >::importanceWeighting(){
  ...
  for(int i = 0; i < this->nParticles_; i++){
    ...
    double measurementLikelihood = rfsMeasurementLikelihood( i, config.importanceWeightingEvalPointCount_ );
...
rfsMeasurementLikelihood( const int particleIdx, const int maxNumOfEvalPoints ){
  ...
  while (likelihood == 0){
    ...
    if( likelihood == 0 ){
      ...
      extraClutterRows++;
      printf("Adding clutter row %d for RFS measurement likelihood calculation", extraClutterRows);

...
def ssc(keypoints, num_ret_points, tolerance, cols, rows):
    ...
    while not complete:
        ...
        if (
            ...
            result_list = result  # return the keypoints from the previous it.
            break
        ...
        covered_vec = [
            [False for _ in range(num_cell_cols + 1)] for _ in range(
                num_cell_rows + 1)
        ]
        result = []
                ...
                for row_to_cover in range(row_min, row_max + 1):
                    for col_to_cover in range(col_min, col_max + 1):
                        if not covered_vec[row_to_cover][col_to_cover]:
                            # cover cells within the square bounding box with
                            # width w
